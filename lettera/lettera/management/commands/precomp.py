...
before_passato = [ s.lower() for s in [ 'e', 'era', 'verno', 'Carlo', 'sendo', 'già', 'è', 'abbino', 'ancora', 'avevano', 'manca', 'avea', 'sia', 'quando', 'sendo', 'poi', 'abbino', 'cosi', 'avendo', 'Andrea', 'proxime', 'fussi', 'ha', 'che', 'essere', 'essendo', 'Spedalingo', 'Signoria', 'ora', 'fussi', 'tornò', 'avere', 'mattina' ]]
before_passati = [ 'nuovo', 'sono', 'essere', 'prossime', 'sete', 'erano', 'fussino', 'ché', 'sendo', 'avventura', 'ancora', 'e', 'fanno', 'questa']
before_passate = [ 'erano', 'sono', 'sieno', 'essere' ]
before_passata = [ 'alla', 'nella', 'è', 'state', 'essendo', 'quella', 'questa', 'fussi', 'della', 'piena', 'la', 'quale' ]
...
class Command(BaseCommand):
  ...
  def handle(self, *args, **options):
    ...
    occurrences = Occurrence.objects.filter(Q(word__name='stato') | Q(word__name='stati')  | Q(word__name='passati')  | Q(word__name='passato') | Q(word__name='passata') | Q(word__name='passete'))
    ...
    for s in sentences:
      ...
      for x, y in pairwise(phrase):
        if (y.word.name == 'passato' and x.word.name in before_passato
         or y.word.name == 'passati' and x.word.name in before_passati
         or y.word.name == 'passate' and x.word.name in before_passate
         or y.word.name == 'passata' and x.word.name in before_passata):
    ...
    participes = Occurrence.objects.filter(tag__name="passare").update(family=passare)
